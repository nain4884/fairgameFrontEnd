...
export default function IndiaPakLiveBookMaker({ add, match, Bid }) {
  ...
  const { bookmakerTeamRates } = useSelector(
  ...
  const AddSession = () => {
    ...
    async function getManuallBookMaker(id) {
      try {
        ...
        console.log("heyyy", response.data.data.marketName);
        if (response?.data?.data?.length === 0) {
          ...
          setLTeamARate(response?.data?.data.teamA_Back);
          setLTeamALayValue(response?.data?.data.teamA_lay);
          setLTeamBRate(response?.data?.data.teamB_Back);
          setLTeamBLayValue(response?.data?.data.teamB_lay);
          setLTeamCRate(response?.data?.data.teamC_Back);
          setLTeamCLayValue(response?.data?.data.teamC_lay);
          // alert(id)
          getAllBetsData(response?.data?.data.id, id);
          setteamRates({
          ...
          setIsTeamASuspend(response?.data?.data.teamA_suspend ? true : false);
          setIsTeamBSuspend(response?.data?.data.teamB_suspend ? true : false);
          setIsTeamCSuspend(response?.data?.data.teamC_suspend ? true : false);
    ...
    const handleSuspend = () => {
      if (
        !teamSuspend.teamA_suspend ||
        !teamSuspend.teamB_suspend ||
        !teamSuspend.teamC_suspend
      ) {
      ...
      <>
        ...
        >
          ...
          >
            ...
            <Box
              sx={{ position: "absolute", zIndex: 999, top: "40px", right: 0 }}
            >
              {visible && (
                <ResultComponent
                  onClick={() => {
