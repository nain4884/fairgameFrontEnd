...
const SideBar = ({ mobileShow }) => {
            ...
            <Box onClick={() => {
                ...
                <Box sx={{ display: 'flex', height: '100%' }} >
                    ...
                    <Box sx={{ display: 'flex', flex: 1, height: '100%', justifyContent: { mobile: "flex-start", laptop: 'center' }, alignItems: 'center' }} >
                        <Typography variant="menuListHeader" sx={{ fontSize: { laptop: showSideBarMobile ? "18px" : "16px", mobile: "20px" }, fontWeight: { mobile: "500", laptop: "600" } }}>{title}</Typography>
                    </Box>
    ...
    const ListItem = ({ item }) => {
        return (
            <Box sx={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', height: '5vh', minHeight: '20px', width: { mobile: '100%', laptop: "60%" } }} >
                <Typography variant="menuListItem" sx={{ fontSize: { laptop: showSideBarMobile ? "15px" : "13px", mobile: "20px" }, marginLeft: { mobile: "35px", laptop: '0px' } }} >{item}</Typography>
            </Box>
        )
    }
    ...
    return (
        <Box sx={[{ width: { laptop: showSideBarMobile ? "100%" : '18%', mobile: mobileShow ? "100%" : "0%" }, minHeight: '500px', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'auto', alignItems: { mobile: 'flex-start', laptop: "flex-end" }, backgroundImage: `url(${drawerBackground})` }]}>
