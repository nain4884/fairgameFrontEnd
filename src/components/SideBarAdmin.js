...
const RenderDates = ({ i,handleDrawerToggle }) => {
    ...
    return (
        <Box
            ...
            <MainBox sub={i?.sub} selected={selected} under={true} color={colors[2]} width={80} title={i.title} />
            {
                selected && i?.values?.map((value, index) => {
                    return (
                        <RenderValues handleDrawerToggle={handleDrawerToggle} i={value} k={index} />
                    )
                })
            }
        </Box>
...
const RenderValues = ({ i ,handleDrawerToggle}) => {
    ...
    return (
        <Box
            ...
            sx={{ width: '100%', display: 'flex', alignSelf: 'flex-end', flexDirection: 'column' }} >
            <MainBox sub={i?.sub} selected={selected} under={true} color={colors[3]} width={75} title={i.title} />
            {
                selected && i?.values?.map((value, index) => {
                    return (
                        <RenderBets handleDrawerToggle={handleDrawerToggle} i={value} k={index} />
                    )
                })
            }
        </Box>
...
const RenderEvents = ({ i ,handleDrawerToggle}) => {
    ...
    return (
        <Box
            ...
            sx={{ width: '100%', display: 'flex', alignSelf: 'flex-end', flexDirection: 'column' }} >
            <MainBox sub={i?.sub} selected={selected} under={true} color={colors[0]} width={85} title={i.title} />
            {
                selected && i?.values?.map((value, index) => {
                    return (
                        <RenderDates handleDrawerToggle={handleDrawerToggle} i={value} k={index} />
                    )
                })
            }
        </Box>
...
const RenderGames = ({ i, k ,handleDrawerToggle}) => {
    ...
    return (
        <Box onClick={(event) => {
            ...
            <MainBox sub={i?.sub} selected={selected} under={true} color={colors[1]} width={90} title={i.title} />
            {
                selected && i?.values?.map((value, index) => {
                    return (
                        <RenderEvents handleDrawerToggle={handleDrawerToggle} i={value} k={index} />
                    )
                })
            }
        </Box>
